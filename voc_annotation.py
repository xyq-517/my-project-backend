...
#-------------------------------------------------------#
#   直接指定训练集、验证集、测试集占总数据集的比例
#   train_percent + val_percent + test_percent = 1.0
#-------------------------------------------------------#
train_percent       = 0.7
val_percent         = 0.2
test_percent        = 0.1
...
if __name__ == "__main__":
    ...
    num     = len(total_seg)  
    indices_range = range(num)  # 修改变量名，避免覆盖内置函数list
    
    # 确保比例总和为1
    if abs(train_percent + val_percent + test_percent - 1.0) > 1e-6:
        raise ValueError("训练集、验证集、测试集的比例之和必须为1")
    ...
    # 随机划分数据集
    indices = list(range(num))  # 使用内置的list函数
    ...
    for i in indices_range:  # 使用新的变量名
        name = total_seg[i][:-4]+'\n'  
    ...
    for i in tqdm(indices_range):  # 使用新的变量名
        name            = total_seg[i]
